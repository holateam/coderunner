...
###Анализатор частоты запросов

Для каждого пользователя, при первом его запросе на сервер создается лог частоты его запросов на сервер.
Файл лога именуется еледующим образом: `~/logfolder/requests/userId`. (есть идея поднять с этой целью бд).
